// gatewayClient.ts —— Gateway Client（PLAN-2.0.md Commit 07）
//
// 契约（§二 硬规则 13 / §五 IPC 与错误码 / §六 AI 调用链 / §七 Commit 07 / §九 待办 #14、#15）：
//   - **硬规则 13**：`GATEWAY_TOKEN` 与 Gateway HTTP 调用**只存在于主进程**。本模块是应用里
...
//   - **探活 → 按需自动拉起 → 就绪轮询**：探活（`GET /health` 200，§六 实测 97ms）失败才叫
//     starter **一次且只一次**（starter 的实现在主进程 wiring 里复用 clawManager 的启停，
//     见 electron/main/index.ts —— §七 原文「复用 clawManager 启停，不另起炉灶」）。
//   - **SSE → IPC 透传**：`createChatStream()` 给出增量迭代器 + `cancel()`，
//     `forwardGatewayStream(webContents, streamId, iterator)` 把增量按事件推给渲染进程。
//   - **会话隔离**：请求体 `user` 一律由本模块拼装为 `conv:<projectId>:<conversation_key>`
//     （§六），调用方给不出 conversation_key 时**报错而不是退回 projectId**
//     （否则多个 Project 会共用同一个 OpenClaw sticky 会话，Memory 串味）。
//   - **多模态模型选择**：`model` 取值只能是 `openclaw` 或 `openclaw/<agentId>`
//     （§六 实测：不是 provider 模型 id）。含图请求走 `multimodal` 模型；**未配置多模态模型
//     时明确报错（不静默降级到纯文本模型）**，否则用户只会看到「AI 看不见我的图」。
//   - 错误沿 §五 码透传，判定上游错误用 `errorCodeOf()` 而不是 `instanceof`
//     （跨 bundle 会假阴性，见 references/node-ts-verification.md）。
//
// 与 §五 IPC 的关系：§五 没有 gateway 面，本提交只注册**两条**只读/幂等通道
// （`marketing:gateway:status` / `marketing:gateway:ensureReady`，见 ipc/gateway.ts），
// 业务流通道（advisor / content）归 08/09，07 不预建。
 ...
 *
 * 08/09 的渲染端按 `streamId` 归并自己的流：chunk* → done | error。
 * `cancel` 刻意**没有**事件：中止由渲染端发起，它自己知道，不需要回执。
 */
export const GATEWAY_STREAM_EVENTS = {
  chunk: 'marketing:gateway:chunk',
  done: 'marketing:gateway:done',
  error: 'marketing:gateway:error'
} as const
...
/** §六 会话隔离键前缀：user = `conv:<projectId>:<conversation_key>` */
...
export interface GatewayChatInput {
  /** 当前 Project（硬规则 9：显式传参，禁止全局推断） */
  projectId: string
  messages: GatewayChatMessage[]
  /** 会话隔离键；不给则由注入的 `conversationKeyResolver` 解析（主进程从 projects 表取） */
  conversationKey?: string
  /** true = SSE 流式（产品必须走流式：§六 首字 1.27s vs 冷启动 80.3s） */
  stream?: boolean
  /** 显式指定模型（仍受形态白名单约束）；不给则按 text/multimodal 规则选 */
  model?: string
  temperature?: number
  /** 外部中止信号（组件卸载 / 切换商家）；与 `handle.cancel()` 等价 */
  signal?: AbortSignal
}
...
/** §六 会话隔离键：`conv:<projectId>:<conversation_key>`（含注入防护） */
export function buildConversationUser(projectId: string, conversationKey: string): string {
  const pid = typeof projectId === 'string' ? projectId.trim() : ''
  const key = typeof conversationKey === 'string' ? conversationKey.trim() : ''
  if (!pid) {
    throw new AppError(ERROR_CODES.VALIDATION_ERROR, '缺少 projectId（会话隔离键无法拼装）', { field: 'projectId' })
  }
  if (!key) {
    throw new AppError(ERROR_CODES.VALIDATION_ERROR, '缺少 conversation_key（会话隔离键无法拼装）', {
      field: 'conversationKey'
    })
  }
  // 任一字段含 ':' 都能伪造出别人的会话边界（跨 Project 串 Memory），必须拒
  if (pid.includes(':') || key.includes(':')) {
    throw new AppError(ERROR_CODES.VALIDATION_ERROR, "projectId / conversation_key 不允许包含 ':'", {
      field: pid.includes(':') ? 'projectId' : 'conversationKey'
    })
  }
  return `${CONVERSATION_USER_PREFIX}${pid}:${key}`
}
...
export class GatewayClient {
  ...
  /**
   * 只读快照（`marketing:gateway:status`）：**只发 GET /health + GET /v1/models，零 token**，
  ...
  /** 解析 `conversation_key`：显式给了就用，否则走注入的解析器（主进程从 projects 表取） */
  async resolveConversationUser(input: GatewayChatInput): Promise<string> {
    ...
    if (explicit) return buildConversationUser(input.projectId, explicit)
    ...
    const key = await this.conversationKeyResolver(String(input?.projectId ?? ''))
    return buildConversationUser(input.projectId, key)
  }
 ...
 *
 * - `marketing:gateway:chunk` → `{ streamId, index, delta }`
 * - `marketing:gateway:done`  → `{ streamId, chunks, text, aborted }`
 * - `marketing:gateway:error` → `{ streamId, error: { code, message, details? } }`
...
export function forwardGatewayStream(
  ...
  const done = (async () => {
    ...
    try {
      ...
      if (cancelled) return
      webContents.send(GATEWAY_STREAM_EVENTS.error, { streamId: id, error: toEnvelope(e) })
    }
  })()
