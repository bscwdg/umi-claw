// test/gateway.accept.mjs —— Commit 01 验收：Gateway Client（探活/拉起/SSE/中止/会话隔离/端点开关）
//
// **打真实源码**：esbuild 把 electron/main/gatewayClient.ts（以及真 DatabaseClient）bundle 成临时 ESM，在纯 Node 里 import；HTTP 侧用**真 Node http 服务端**
...
//
// 覆盖点（对齐 PLAN-3.0.md §14.1 / §14.2 / §6.4 / §6.5 / 硬规则 3）：
...
try {
  ...
  await r.check('G1', 'gatewayClient.ts 可纯 Node import；端点/事件名/超时契约在位；缺依赖即炸', async () => {
    ...
    assertEq(GATEWAY_STREAM_EVENTS.chunk, 'work:stream:chunk', 'chunk 事件名')
    assertEq(GATEWAY_STREAM_EVENTS.done, 'work:stream:done', 'done 事件名')
    assertEq(GATEWAY_STREAM_EVENTS.error, 'work:stream:error', 'error 事件名')
    // §六 时序事实：控制面短、数据面等得起
  ...
  await r.check('G5', '错误映射：401→AUTH / 400→VALIDATION / 5xx→NOT_READY(upstream)', async () => {
    ...
    for (const [mode, expected] of cases) {
      ...
      const res = await outcome(
        client.chat({ messages: [{ role: 'user', content: 'hi' }], conversationKey: 'conv:work:qa' })
      )
  ...
  // ── G10 会话隔离 ──
  await r.check('G10', '会话隔离：user=调用方给的完整会话键（§6.4）+ 形状校验（防注入）', async () => {
    // 3.0 无 Project 概念（2.0 的 conv:<projectId>:<key> 已废除）：
    // 调用方直接给完整键，客户端只做**形状校验**，绝不拼装、绝不让渲染端伪造边界。
    const client = makeClient(baseUrl)
    ...
    const body = fake.state.chatBodies[fake.state.chatBodies.length - 1].body
    assertEq(body.user, 'conv:work:qa', 'user 必须是调用方给的完整会话键')
    assertEq(CONVERSATION_USER_PREFIX, 'conv:', '前缀应为 conv:')

    // 三类键（§6.4）都必须被接受
    for (const key of ['conv:work:qa', 'conv:work:tool:minutes', 'conv:work:report:daily:2026-09-23']) {
      const ok = await outcome(Promise.resolve().then(() => assertConversationKey(key)))
      assertEq(ok.ok, true, `合法键应通过: ${key}`)
    }
    // 形状非法一律拒（空白/换行/大写/非 conv 前缀/空）
    for (const bad of ['', '   ', 'work:qa', 'conv:work qa', 'conv:work\nqa', 'conv:Work:QA', 'conv:work:qa;DROP']) {
      const res2 = await outcome(Promise.resolve().then(() => assertConversationKey(bad)))
      assertEq(res2.code, 'VALIDATION_ERROR', `非法键应被拒: ${JSON.stringify(bad)}`)
    }
    // 缺 conversationKey → 明确报错（不退回任何默认会话）
    const missing = await outcome(client.chat({ messages: [{ role: 'user', content: 'hi' }] }))
    assertEq(missing.code, 'VALIDATION_ERROR', '缺 conversationKey 应 VALIDATION_ERROR')

    // 渲染端拿不到 token
    const preloadSrc = readFileSync(join(repoRoot, 'electron', 'preload', 'index.ts'), 'utf-8')
    assert(!/GATEWAY_TOKEN/.test(stripComments(preloadSrc)), 'preload 不得引用 GATEWAY_TOKEN（硬规则 3）')
    assert(!/Bearer\s/.test(stripComments(preloadSrc)), 'preload 不得出现 Bearer（调用只留主进程）')
    return `user=conv:work:qa（原样透传）；3 类合法键通过；7 个非法形状被拒；preload 无 token`
  })
  ...
  await r.check('G16', '透传助手：chunk*N → done 事件序；cancel 后静默；错误走 error 事件；销毁自动中止', async () => {
    ...
    assertEq(events[0].payload.runId, 'stream-1', '事件应带 runId（v0.7 统一，无 streamId）')
  ...
  await r.check('G22', '模型选择**按次解析**：配置从「无多模态」变「有多模态」后同一个 client 立刻生效', async () => {
    ...
    const before = await outcome(client.chat({ conversationKey: 'conv:work:qa', messages: imageMessages }))
    ...
    const after = await outcome(client.chat({ conversationKey: 'conv:work:qa', messages: imageMessages }))
    ...
    const snap = await snapClient.chat({ conversationKey: 'conv:work:qa', messages: imageMessages })
  ...
  await r.check('G24', '调用失败后作废就绪缓存：ensureReady 重新探活/拉起（不再回陈旧快照）', async () => {
    ...
    const failed = await outcome(client.chat({ conversationKey: 'conv:work:qa', messages: [{ role: 'user', content: 'x' }] }))
 ...
 // ── G17 端点开关默认化 ──
  ...
  await r.check('G20', 'wiring：两条只读通道 + preload 面 + 主进程复用 clawManager + token 不出主进程', async () => {
    ...
    assert(ipcSrc.includes("'work:gateway:status'"), '应注册 work:gateway:status')
    assert(ipcSrc.includes("'work:gateway:ensureReady'"), '应注册 work:gateway:ensureReady')
    // 本提交只提供透传助手，不注册业务流通道（业务通道归各自 Commit）
    assert(
      !/ipcMain\.handle\(\s*'work:gateway:(chunk|done|error|chat|stream)/.test(ipcSrc),
      '本提交不应注册业务流通道（只提供 forwardGatewayStream 助手）'
    )
    assert(ipcIndexSrc.includes('registerGatewayIpc'), 'ipc/index.ts 应转出 registerGatewayIpc')
    assert(preloadSrc.includes("'work:gateway:status'"), 'preload 应暴露 status')
    assert(preloadSrc.includes("'work:gateway:ensureReady'"), 'preload 应暴露 ensureReady')
    assert(/work:\s*\{/.test(preloadSrc), 'preload 应有 work.gateway 面')
    // 主进程：starter 复用 clawManager（不另起炉灶）+ token 取自 openClawPaths
    assert(/createWorkGatewayClient/.test(mainSrc), 'main/index.ts 应构造 Gateway Client')
