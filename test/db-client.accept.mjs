...
function makeClient(overrides = {}) {
  const client = new DatabaseClient({
    ...
    workerScriptPath,
    nodePath,
    subprocessName: 'marketing-db-worker-test',
...
try {
  ...
  await r.check('C5', '断线自动重启一次：读自动重试成功，写不自动重试（DB_ERROR）', async () => {
    const now = Date.now()
    await main.request('projects.upsert', {
      data: { id: 'pc1', name: '压测店', conversation_key: 'kc1', created_at: now, updated_at: now }
    })
    ...
    const writeP = main.request('projects.upsert', {
      data: { id: 'pc2', name: '写不重试', conversation_key: 'kc2', created_at: now, updated_at: now }
    })
    await sleep(200)
    const readP = main.request('projects.list', {})
    await sleep(200)
  ...
  await r.check('C9', 'backup() VACUUM INTO + 保留最近 5 份（超出删最旧）', async () => {
    ...
    const snap = direct(join(backupDir, files[files.length - 1]), (db) =>
      db.prepare("SELECT COUNT(*) AS c FROM sqlite_master WHERE type='table' AND name='projects'").get().c
    )
    assertEq(snap, 1, '快照应含 projects 表')
  ...
  await r.check('C11', '失败信封 code 全部落在 §五 错误码表内', async () => {
    ...
    assert(allowed.has(bad.code), `错误码应来自 §五: ${bad.code}`)
    const nf = await main.request('projects.get', { keys: { id: 'ghost' }, required: true }).then(
      () => ({ ok: true }),
      (e) => ({ ok: false, code: e.code })
    )
  ...
  await r.check('C12', '子进程注册表接入：启动即注册，gracefulStop 走优雅退出', async () => {
    ...
    assertEq(list[0].name, 'marketing-db-worker', '注册名应对')
