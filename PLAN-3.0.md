# Umi Claw 3.0「工作版」规划基线（v0.17 — 待办到点提醒·本机必达篇）

> 本文档是 3.0 的唯一规划基线，随开发进度持续更新（沿用 `PLAN-2.0.md` 模式）。
> 状态：**规划冻结（v0.9–v0.17 局部解冻：提醒外发 + 冷启动弹窗 + 待办到点提醒，见修订记录）——产品/系统/工程/数据/IPC/合规全部钉死，开工前终检 8 项已清** ｜ 更新日期：2026-09-24 ｜ 分支：`release/3.0`（基于 `release/1.0`）
...
> v0.9 提醒外发局部解冻（**仅提醒一项，北 2026-09-23 拍板**）：①**硬规则 22 重评估并收窄**——放开「时刻可自定义」与「到点自动生成草稿 + 外发短提示」，仍**禁止**自定义规则 / 重复规则 / 多提醒 / 日历同步（护栏收窄，不拆除）②**§八 时刻不再写死**（09:00 / 18:30 降为**默认值**，用户可改，存 `app_meta`，不新增表）③**外发采用方案 A**：只推「草稿已就绪」**短提示**，**绝不推日报正文**（正文仍需人工确认，硬规则 5 / 20）；外发**默认关**，未配齐通道 + 目标不得开启 ④**§14 契约补 `wizard` / `reminder`**（Commit 09 已落地却未回写契约，违反 §19 规则 4；命名空间 12 → **14**）⑤新增 IPC `work:reminder:{getTimes, setTime, getPushConfig, setPushConfig}`。
...
| v0.9 | 2026-09-23 | **提醒外发局部解冻（仅此一项，功能冻结其余部分不变）**：①**硬规则 22 重评估并收窄**——允许「时刻可自定义」+「到点自动生成日报草稿」+「外发短提示」，仍禁自定义规则 / 重复规则 / 多提醒 / 日历同步 ②**§八 时刻从写死改为默认值**（09:00 / 18:30 为默认，用户可在「工作设置」改；存 `app_meta`，守硬规则 24 不新增表）③**外发方案 A**：只推短提示（= 本地通知同一段 title + body），**构造上不可能带日报正文**；正文仍需人工在「报告」页确认（硬规则 5 Human-in-the-loop / 20 可纠错）；**外发默认关**，未配齐通道 + 目标时 `setPushConfig({enabled:true})` 直接 `VALIDATION_ERROR`（不给「开了但不知道发哪」的状态）④外发实现走主进程 spawn OpenClaw 渠道 CLI（复用 gateway 启动同一套 `buildOpenClawEnv`，不另造路径）；失败**不抛**、不阻断本地通知（尽力而为）⑤**§14 契约补 `wizard` / `reminder` 两命名空间**（Commit 09 落地未回写，违反 §19 规则 4「契约先于代码」；命名空间 12 → 14）⑥验收：`wizard.accept` 新增 M5（时刻可改）/ M6（本地闭环）/ M7（方案 A 外发） |
| v0.10 | 2026-09-24 | **外发扩展（仍仅提醒一项，北 2026-09-24 拍板）**：①**下拉只展示已配置渠道**——「没配置肯定不能推」，未配齐凭证的渠道不出现（后端也拒：`push-channel-not-configured`）②**渠道清单 4 项**：飞书 / 企业微信 / 微信（`openclaw-weixin`）/ 钉钉；其中**钉钉 `supported=false`**——OpenClaw 无任何钉钉渠道实现（docs/dist/插件目录全盘搜索为空），**永远不可选**，保留在清单只为在提示里说明「为什么不给选」③**首选 + 次选（兜底）**：首选失败自动接着试次选；首选与次选不得相同；选了次选必须填次选目标④**失败提示**：最近一次外发结果（成功/失败 + 原因 + 时间）存 `app_meta`（守硬规则 24 不新增表），设置页可见⑤新增 IPC `work:reminder:{availablePushChannels, getPushStatus}`；`setPushConfig` 参数扩 `fallbackChannel` / `fallbackTarget`⑥「去配置渠道」按钮跳 `/channelsPage`⑦验收：`wizard.accept` 新增 M8（清单/兜底/失败提示） |
| v0.11 | 2026-09-24 | **外发目标自动发现 + 推送测试（北 2026-09-24 拍板）**：①**用户不再手填目标**——「首选目标是什么，用户怎么知道，不需要填，openclaw 自己知道」。目标改为从 OpenClaw 自己记录的 **agent 库 `conversations` 表**（`channel` + `delivery_target` + `kind`）读出，UI 只展示友好标签（如「私聊」「群聊」），不暴露原始 ID②**推送测试按钮**：「发送测试消息」按已配置的**首选→次选**发一条探针消息（不含任何工作内容），成败即时回显并写入 `pushStatus`（与正式外发同一套兜底逻辑）③新增 IPC `work:reminder:{availablePushTargets, testPush}`④读 agent 库走**便携 node 子进程 + `node:sqlite` 只读**（同 `clawManager` 先例：Electron 30 内置 node 未必启用 `node:sqlite`）⑤验收：`wizard.accept` 新增 M9（目标发现/测试推送/兜底/失败入状态） |
| v0.12 | 2026-09-24 | **用户协议闸门（北 2026-09-24 拍板）**：①**首启向导的同意步骤改为不可关闭的阻断弹窗**——必须勾选同意才能关闭并继续使用应用；**只出现一次**（同意后向导内不再重复问，直接从画像步开始）②**设置页新增「用户协议」区**：已同意则打勾并显示同意时间；点「用户协议」几个字**重新弹窗并反显之前填的**（已同意则默认打勾 + 展示「你已于 X 同意」）③**启动 OpenClaw 前校验同意**（硬性闸门）：未同意时 `claw:start` / `claw:restart` / gateway 自动拉起 / 托盘菜单启动全部拒绝，返回 `reason: consent-required`④新增 IPC `work:wizard:ensureConsent`；`WizardStatus` 增 `consentAt`（同意时间，守硬规则 24 不新增表）⑤**推送测试前置**：OpenClaw 未运行时不发测试，提示「请先启动 OpenClaw」+「去控制台启动」按钮⑥验收：`wizard.accept` Z2 扩写（闸门拒/放行、同意时间回读、重复同意保留首次时间）；UI 验收 harness 补子组件桩（单页验收只编译被挂页，子组件 .vue 换桩） |
| v0.13 | 2026-09-24 | **冷启动向导改弹窗（北 2026-09-24 拍板）**：①问题——向导已改弹窗但**路由仍在**，侧边栏「开始向导」还能点、页面还能跳，两套并存形同虚设。北：「不要路由了，修改为弹窗」②**向导从路由页改为弹窗**：新建 `src/views/components/WizardModal.vue`（与用户协议弹窗同构：**不可关闭**、无 ✕、点遮罩不关，走完或逐步跳过才 `complete()`）；**删除** `src/views/work/WizardPage.vue` 与路由 `/work/wizard`；**侧边栏「开始向导」入口一并移除**③新增 `src/composables/useWizardGate.ts`（模块级 `wizardVisible` ref 单例）供 App 首启自检与 Setup 完成后接续两处共享——**不用 pinia**，否则组件级验收（无 pinia 实例）挂不起页④向导完成后关弹窗并跳「今日」⑤验收：U8 改为挂载 `WizardModal.vue`⑥**弹窗层级与居中修正（北 2026-09-24 复查）**：向导弹窗原 `z-index:2900` 压过了「关闭 Umi Claw」确认框（2100）—— 用户点关闭会看不到确认框、无法退出；现钉死层级序 **向导 1900 < 协议 1950 < 模型选择 2000 < 关闭确认 2100**，两个新弹窗均降到确认框之下；同时把向导由 `align-items:flex-start` 改为**水平垂直居中**（`align-items/justify-content:center` + 卡片 `margin:auto`，兼顾内容超高时不被顶部裁切） |
| v0.14 | 2026-09-24 | **协议联动 + 可重测（北 2026-09-24 拍板）**：①问题——设置页「用户协议」的勾选框是写死的 `disabled`（纯装饰），**与弹窗没关联**；且同意后弹窗只出现一次，**无法再测**。北：「这个要和那个弹窗关联呀，我现在启动没有那个弹窗了怎么测试」②新增 `src/composables/useAgreementGate.ts`（模块级单例：`agreementVisible` / `agreementBlocking` / `consentGranted` / `consentAt`）—— 弹窗全App**唯一实例**（渲染在 App.vue），首启自检与设置页入口**读同一份状态**，勾选框与弹窗从此联动；不用 pinia（组件级验收无 pinia 实例）③设置页不再自行渲染弹窗（避免双遮罩）④新增 IPC `work:wizard:revokeConsent`（清同意标记与时间，**不动 completed**）+ 设置页「撤销同意（重新测试）」按钮，供随时重验首启流程⑤验收：`wizard.accept` 新增 Z6（撤销后闸门重新生效 / 可再次同意且取新时间 / completed 不变）；UI 验收 harness 的 composable 别名改为**自动扫描目录**（此前写死白名单，新增 composable 必挂 U6，已踩三次） |
| v0.15 | 2026-09-24 | **向导入口回归（北 2026-09-24 拍板）**：①问题——v0.13 删掉侧边栏「开始向导」后**没在别处补入口**，向导弹窗仅在首启未完成时自动弹，完成后就再也打不开。北：「那开始向导呢去哪了，弹窗哪去了」②**设置页「用户协议」卡片内新增「开始向导」按钮**（北指定位置）→ `openWizard()` 开弹窗；「撤销同意」保留并移入同一按钮组③`WizardModal` 新增 `watch(visible)`：**每次打开都重新初始化**（否则常驻挂载下完成后重开仍停在完成态、看不到步骤），`onMounted` 只首次初始化④验收：UI 11/11、wizard 16/16、全量 13/13 |
| v0.16 | 2026-09-24 | **待办定时外发 + 到期精确到分钟 + 删除二次确认（北 2026-09-24 拍板；总开关口径同日二次拍板）**：①**硬规则 22 再收窄**——允许**待办逐条带一个外发时刻**（`todos.remind_at`），复用提醒那**同一个** 30s check 循环到点投递；仍不引入 cron 框架、不加通知种类、不做重复规则。**受「外发总开关」管**：关着**不发也不消耗**（不写 `reminded_at`），开启后下一轮补发②**schema v1 → v3**：`MIGRATION_STEPS` 追加 v2（`remind_at` / `reminded_at` + `idx_todos_remind`）与 v3（`due_at`）；**v1 建表语句已发布、冻结不改**，新列只走迁移 ALTER，新索引也不塞进 v1 的 `INDEX_STATEMENTS`；`db-worker` 列白名单同步③**修「迁移前快照」自等死锁**：`backup()` 内部 `await ensureReady()`，而迁移发生在 `startWorker()` 之内 → 拿回本轮尚未兑现的 `readyPromise` → 永久挂住。**只有「已有库真实升级」这条路径才走到**（空库建库 `from=0` 不备份），故此前验收全绿；现拆出私有 `backupDuringInit()` 直连 `sendRaw`，并补 **C16** 锁住④**到期精确到分钟**：`due_at`（epoch ms）与 `due_date` **双列同步**（给了 `due_at` 就由它推导 `due_date`；`due_date` 不废弃，按天聚合/索引/今日页仍走它）；`due_at` **允许过去**（补登逾期），`remind_at` **必须晚于当前时间**，且**改期即清 `reminded_at`**（否则投递过一次的待办改到任何时间都不会再发——已踩）⑤**「今日」页改名「新的一天」**（路由仍 `/work/today`）；速记入口**只专注记待办**：回车**永不跳页**（`route()` 结果仅用于提取 `draft.title` / `draft.dueDate`），问答走独立「💬 去工作问答」按钮；**到期时间必填**（默认今天 18:00，过点顺延明天，每次挂载重算）；勾选「📩 外发提醒到渠道」+ 发送时间；外发未就绪（未配通道 / 总开关关）时只给「去配置 / 去开启」入口跳 `工作设置#push`，**不调度**（待办本身照建）⑥**删除二次确认**：工作记录 / 事项 / 知识资料统一走 `ConfirmDialog`（点删除只弹窗、确认才删、取消零调用；事项删除说明「记录保留、仅解绑」）⑦**外发失败可见且可归因**：`pushStatus` 是全局单条，故失败信息带来源前缀 `待办「标题」：原因`；连「通道未配齐」这种一次都没投出去的兜底分支也要写状态（原先静默）⑧**扫描不吃 worker 默认 limit**：`listDueReminders` 显式 `limit: 5000`（`clampLimit` 默认只给 500 行，否则较老的到点待办会被静默漏掉）⑨验收：`db-client` 新增 **C16**（v1 带数据库 → v3：不挂死 / 迁移前快照 / 老数据完好 / 新列可写）、`work` 新增 **T10**（到点判定 / 标记去重 / 改期重新排队 / 505 条填充下仍扫到最老那条）、`wizard` 新增 **M10**（总开关管住 + 开启补发 / 到点发一次 / 改期再发 / 单条异常不连累 / 失败可见含脏 meta 兜底）、`ui` 新增 **U13/U14**（记录·事项·知识删除二次确认）、U12 扩到「未配 / 开关关都拦住」并**修掉写死日期的时间炸弹**；harness 支持编译**真子组件**（`realComponents`）+ 补 v-model / transition 所需最小 DOM 桩⑩四处修复均以**变异测试**验证（撤回修复 → 对应用例必红） |
| v0.17 | 2026-09-24 | **待办到点提醒改判：本机通知必达 + 外发降为加成（北 2026-09-24 三次拍板）**：①**问题**——v0.16 把待办的 `remind_at` 只接到「外发到渠道」，于是**没配渠道 / 总开关关着的人到点什么都不会发生**（只在设置页留一条失败记录），而「我设了时间就该有反应」是用户的默认预期；且本地通知能力本来就有（`Notification`），不用白不用②**新口径三条**：**本机通知必达**（到点一定弹，不看外发总开关、不看通道配置、甚至不需要注入 pusher）/ **外发是加成**（总开关开 + 通道配齐才顺带投渠道，失败落 `pushStatus` 不重试）/ **到点即消费**（弹过就写 `reminded_at`，**不补发**——宁可少一次外发，也不要每 30s 重复弹通知；故标记紧跟本地通知之后、外发之前）③**推翻 v0.16 的「关着不消耗、开启后补发」**：本机既然已经按时提醒过，事后再补一条外发只会变成骚扰；补发语义同时消掉了「关几天再开就一次性轰出一批」的怪现象（也不再需要第二个标记列 → 守硬规则 24，schema 仍停在 v3）④**硬规则 22 再收窄一次**：本地通知从「两个固定」变为**三类**（早上汇总 / 日报草稿 / **待办到点提醒**）；仍禁 cron 框架 / 自定义规则 / 重复规则 / 多提醒种类 / 日历同步；待办提醒**不进** `REMINDER_IDS`（无时刻设置、无独立开关、不参与固定通知的当天去重，去重靠 `reminded_at` 落库）⑤**渲染端不再拦截**：「新的一天」页勾选框改名 **「⏰ 到点提醒我」**，外发未就绪（未配通道 / 总开关关）**照样调度**，只把弱提示改成「未配置外发渠道（到点仍会本机通知）/ 外发开关未开启（到点仍会本机通知）」+「去配置 / 去开启」入口；设置页总开关文案同步改为「只管外发到渠道，待办提醒到点仍会在本机弹通知」⑥**术语统一**：全仓 `定时外发` → **`到点提醒`**（含 v2 迁移步骤名、列注释、校验报错文案「提醒时间必须晚于当前时间」、`TodayTodoItem.remindAt` 注释、列表徽标 `📩` → `⏰`）⑦验收：M10 重写为 5 段（**总开关关着也弹本机通知且不补发** / 开启后本机 + 外发各一次且不重复 / 改期重新排队 / 单条异常不连累（异常那条已弹通知但未标记）/ 外发失败与「通道 meta 被清空」兜底都写可见状态并点名待办 / **完全没接 pusher 也必达**）；U12 第 5 步改为断言**未就绪仍带 `remindAt`**；两处均以**变异测试**验证（撤回 `notifier` 调用 → M10 红；渲染端恢复「未就绪不调度」→ U12 红）⑧**本机通知不得抛**：`showReminderNotification` 加 `Notification.isSupported()` 判定 + try/catch——本机通知既已是**必达主通道**，抛出去会被逐条兜底当成「未消费」而每 30s 重试一次，结果是永远弹不出来还一直刷日志；不可用则记日志放过（提醒照样消费）。Windows toast 依赖的 `setAppUserModelId('com.clawdesktop.app')` 已在 `app.whenReady()` 内设置，与打包 `appId` 一致 |
...
- **产出型 / 加工型分流**：只有**产出型** AI 动作（生成纪要/日报/**邮件起草**等有交付物的）才产生候选；**加工型**（润色/翻译/摘要/**邮件润色**——加工一个已有片段）不产生。判断依据是「这次动作有没有产生关于『干了什么活』的新事实」，**邮件按「起草 / 润色」两种交互分别归类，不一刀切**。
...
| `todos` | 待办 | `title`、`due_date?`、**`due_at?`（v0.16：精确到期时刻，epoch ms）**、`matter_id?`、`source(manual/extracted/routine)`、`routine_rule?(daily/weekly)`、`state(candidate→confirmed(open)→done)`、`done_at`、**`remind_at?` / `reminded_at?`（v0.16 加列，v0.17 改判语义：到点提醒时刻 / 已提醒标记，弹过即消费）** |
...
> **当前 `SCHEMA_VERSION = 3`（v0.16）**：v1 = 8 表建表 + 索引（**已发布，冻结不改**）；v2 = `todos` 加 `remind_at` / `reminded_at` + `idx_todos_remind`；v3 = `todos` 加 `due_at`。**新列只在 `MIGRATION_STEPS` 里 ALTER，绝不回头改 v1 的 DDL**（否则新库与升级库形状不一致）。升级前自动留快照；`user_version` 高于本应用支持值 → 初始化即明确报错，不静默。
...
| `todos` | `INDEX(state, due_date)`、`INDEX(matter_id)`、**`INDEX(state, remind_at)`（v2 迁移步骤里建，不进 v1 的 `INDEX_STATEMENTS`）** | 待办三来源聚合 + 到点提醒扫描 |
...
> **v0.16 改判（北 2026-09-24 拍板）：这一屏的入口从「一句话路由器」收窄为「待办速记」**，页面与侧边栏的「今日」同时改名 **「新的一天」**（路由仍是 `/work/today`，不改路径）。上面那张 IA 图的其余部分不变，输入区变成：
>
> ```text
> ┌────────────────────────────────────────────────────────────────────┐
> │ ┌──────────────────────────────┐ ┌────┐ ┌──────────────┐          │
> │ │ 加个待办，回车记下……          │ │记下│ │💬 去工作问答 │          │
> │ └──────────────────────────────┘ └────┘ └──────────────┘          │
> │ 📅 到期 [2026-09-24 18:00]  ☐ 📩 外发提醒到渠道 [2026-09-24 19:00] │
> │    （未就绪时：⚠ 未配置外发渠道 / 外发开关未开启 → [去配置|去开启]）│
> └────────────────────────────────────────────────────────────────────┘
> ```
>
> - **回车只建待办、永不跳页**：仍调 `router.route()`，但**只取 `draft.title` / `draft.dueDate`** 做 best-effort 提取；任何 `target`（含 qa / 工具 / 报告）都只落待办（验收 U12 断言「零导航」）。问答与工具改走独立按钮 / 侧边栏。
> - **到期时间必填**：默认「今天 18:00」，已过点则顺延明天（避免一建就逾期）；页面每次挂载重算默认值。自然语言里的日期覆盖**日期段**，**时刻仍以选择器为准**；拼不出有效时刻就退回选择器原值（绝不让 NaN 落到主进程）。
> - **到点提醒勾选（v0.17 改判，原名「外发提醒到渠道」→「⏰ 到点提醒我」）**：勾上后出现提醒时间（默认下一整点）。到点**本机通知必达**，与外发配置无关；**外发就绪 = 总开关开 + 首选通道与目标配齐**，未就绪时**照样调度**，只把弱提示改成「未配置外发渠道 / 外发开关未开启（到点仍会本机通知）」+「去配置 / 去开启」入口跳 `工作设置#push`。
> - 待办行徽标：`逾期` / `今天 HH:mm` / `随时` / `📩 MM-DD HH:mm`（外发时刻）。
> - ⚠️ **已知副作用（接受）**：手动速记不再产生「无到期日（随时）」待办——「随时」只剩 AI 提取候选与例事两条来路。
...
6. **「新的一天」页的一句话入口不再导航（v0.16）**：该页输入框**只用于记待办**，`route()` 的结果只用来提取 `draft.title` / `draft.dueDate`；上表「跳工具页 / 跳报告 / 跳问答」的导航行为只在**其它入口**保留。理由：P0 的真实动线是「随手记一件事」，**误判跳页比不跳页更伤**（跳错了要退回来重记）；于是「误判出路」从 v0.4 的「跳过去还能一键转回来」升级为**根本不跳**（第 5 条在工具页仍适用）。
...
下班 18:30  「生成今日日报」高亮（P0 已含本地通知）→ **自动生成草稿** → 事实聚合 → AI 表达 → 草稿
            → 人工修改 → 确认 → 复制走；若当日 confirmed < 3 条 → 漏记追问
            「今天还有没记下的事？」（v0.3 新增，同时是覆盖率的日常机制）
周五 17:00  一键周报：本周记录 + 5 份日报 → 按事项聚合 → 确定性去重（2.5 A7）→ 草稿 → 确认
```

> **时刻是默认值，不是写死的（v0.9）**：上面的 9:00 / 18:30 是**默认时刻**，用户可在「工作设置」里自行修改（存 `app_meta`，不新增表）。

> **P0 提醒（v0.4 上调，v0.9 解冻收窄）**：原「托盘定时提醒」在 P1，但 MVP 假设是「连续 5 个工作日愿意打开」——而第 3–5 天正是掉线高危期。因此 P0 只做**两个本地通知**：早上一次（今日待办汇总）+ 18:30 一次（生成日报），可关，不依赖 Gateway。**时刻可由用户自定义**（v0.9）。其余提醒（记流水/周五周报）留 P1。
>
> ⚠️ **护栏（v0.5 立，v0.9 / v0.16 两次收窄，硬规则 22）**：提醒**仍不是调度系统**——不要加 cron 框架、自定义规则、重复规则、多提醒种类、日历同步。v0.9 放开两项：**时刻可自定义** + **到点自动生成日报草稿**；v0.16 再放开一项：**待办可逐条带一个外发时刻**（复用同一个 check 循环，不新增通知种类）。
>
> **外发（v0.9 新增，方案 A）**：到点可把**同一条短提示**（如「日报草稿已就绪，去确认」）外发到飞书/企微，**绝不外发日报正文**——正文仍需人工在「报告」页确认后才可外传（硬规则 5 / 20）。外发**默认关**；未配齐通道 + 目标不得开启。实现走主进程 spawn OpenClaw 渠道 CLI（复用 gateway 同一套环境），失败不阻断本地通知。
>
> **待办到点提醒（v0.16 新增，v0.17 改判；北 2026-09-24 三次拍板）**：待办可逐条带一个 `remind_at`，到点由提醒那**同一个** 30s check 循环触发：**先弹本机通知（必达），再尽力外发一条短提示**（`待办标题（MM-DD HH:mm 到期）`）到已配置渠道。口径钉死：
>
> - **本机通知必达**：到点一定弹本机通知——**不看外发总开关、不看通道配置、不需要 pusher**（本地通知不依赖 Gateway / 渠道 CLI；没配渠道的人也必须被提醒）。
> - **外发是加成**：`push.enabled=true` + 通道配齐才顺带投渠道；**不看** morning / report 各自的开关（那是两个固定通知自己的事）。
> - **到点即消费**：弹过就写 `reminded_at`，**不补发**（宁可少一次外发，也不要每 30s 重复弹通知）——故标记紧跟本机通知之后、外发之前。外发失败写 `pushStatus`（UI 可见）且**不重试**：渠道被删掉重试也不会成功，瞬态失败由用户手动补发。
> - **改期 = 重新排队**：`todos.update({ remindAt })` 必须同时清 `reminded_at`，否则投递过一次的待办改到任何时间都不会再发（已踩，T10 / M10 锁住）。
> - **失败必须可见且可归因**：`pushStatus` 是全局**单条**记录，故待办外发的失败信息带来源前缀 `待办「标题」：原因`；连「通道未配齐」这种一次都没投出去的兜底分支也要写状态（原先静默丢失）。
> - **逐条兜底**：任一条异常（如列表与标记之间待办被删）不得连累同轮其它到点待办；异常那条不标记，留给下一轮（最坏是多弹一次本机通知）。
> - **例事不重复提醒**：routine 勾完成生成的下一次待办**不带** `remind_at`（守「不做重复规则」）。
> - **扫描必须显式抬 limit**：worker 的 `clampLimit` 默认只给 500 行，不显式传 `limit` 就会静默漏掉较老的到点待办。
...
```text
新的一天    每日聚合入口（待办速记 + 待办 + 记录 + 生成日报）——v0.16 由「今日」改名，路由仍 `/work/today`
...
| AI 工具箱 | 纪要（产出型+待办提取）+ 邮件起草（产出型）+ 润色/翻译/摘要/邮件润色（加工型四件套），**薄边界** |
| 工作知识库 | 2.0 解析管线移植（**预算内全量优先**，见 6.3） |
| 冷启动 | 向导 + 完整度图标 + **隐私告知** |
| **本地提醒** | **两个本地通知**（早上今日待办 + 日报），可关，不依赖 Gateway；**时刻可自定义**（v0.9） |
...
22. **提醒护栏（v0.5 立，v0.9 / v0.16 / v0.17 三次收窄）**：P0 提醒**只做三类本地通知**——两个固定的（早上汇总 + 日报草稿）**+ 待办逐条到点提醒**（v0.16 起，v0.17 定为**本机必达 + 外发加成**），**仍禁止膨胀成调度系统**（不做 cron 框架 / 自定义规则 / 重复规则 / 多提醒种类 / 日历同步）。v0.9 放开两项（北 2026-09-23 拍板）：**时刻可自定义**（存 `app_meta`，默认 09:00 / 18:30）+ **到点自动生成日报草稿**。v0.16 再放开一项（北 2026-09-24 拍板）：**待办可带一个 `remind_at`**，复用同一个 check 循环到点触发；v0.17 改判口径（同日三次拍板）：**到点先弹本机通知（必达，不受外发总开关与通道配置影响），再尽力外发到渠道（受总开关管）**，**到点即消费、不补发**。待办提醒**不进** `REMINDER_IDS`（无时刻设置、无独立开关；去重靠 `reminded_at` 落库）。详见 §八。**外发采用方案 A**：只推短提示（= 本地通知同一段 title + body），**绝不推日报正文**（硬规则 5 / 20）；外发**默认关**，未配齐通道 + 目标不得开启。
...
## 十四、IPC 契约与统一错误规范（v0.6 新增，v0.7 消歧，v0.9 补回写，硬规则 11）
...
window.api = {
  ...
  work: {
    ...
    todos:    { list({ state?, date?, matterId? }),
                create({ title, dueDate?, dueAt?, matterId?, source?, remindAt? }),   // v0.16：dueAt 精确到期 / remindAt 到点提醒（v0.17：本机必达 + 外发加成）
                update(id, { title?, dueDate?, dueAt?, matterId?, routineRule?, remindAt? }),  // 改 remindAt 连带清 reminded_at（重新排队）
                delete(id),
    ...
    today:    { get(date?) },                                          // 一次拉齐：待办三来源 + 记录 + 候选 + 日报状态；待办项含 dueAt / remindAt（v0.16）
    ...
    wizard:   { status(), grantConsent(), revokeConsent(), ensureConsent(), complete(), decide(decision),
                readMapping() },                                       // 冷启动向导 + 旧库三分支 + 启动前同意闸门（v0.9/v0.12/v0.14 补回写）
    reminder: { isEnabled(id), setEnabled(id, enabled),
                getTimes(), setTime(id, hour, minute),
                getPushConfig(), setPushConfig({ enabled?, channel?, fallbackChannel?, target?, fallbackTarget? }),
                availablePushChannels(), availablePushTargets(channel),
                testPush(), getPushStatus(), check() }      // 两个固定通知 + 时刻可改 + 外发（v0.9/v0.10/v0.11）+ 待办到点提醒（v0.16 起，v0.17 改判：**无新通道**，到点队列与回标由主进程注入的 TodoManager.listDueReminders / markReminded 提供；本机通知走同一个 notifier，来源标识 `todo`）
  }
}
...
9. **双时间列同步（v0.16）**：`todos.due_at`（epoch ms，精确到期）与 `due_date`（本地 `YYYY-MM-DD`，按天）是**同一件事的两个粒度**——传了 `dueAt` 就以它为准并**由它推导 `due_date`**（`create` / `update` 同口径）；`due_date` **不废弃**（按天聚合、索引、今日页仍走它）。`dueAt` **允许过去**（补登逾期待办是正当场景）；`remindAt` **必须晚于当前时间**，改期时内部连带清 `reminded_at`。`reminded_at` **不在调用方可改字段里**（只由投递流程写）。
...
13. **真实升级路径（v0.16）**：拿一个**已发布版本的库 + 真数据**跑到当前 `SCHEMA_VERSION`——迁移**不得挂死**（迁移前快照曾在 `startWorker()` 里 `await ensureReady()` 自等死锁，只有这条路径能暴露）、必须留下**迁移前快照**、老数据完好、新列可写（`db-client.accept` C16）。
14. **待办到点提醒（v0.16 起，v0.17 改判）**：**总开关关着也弹本机通知且不补发** / 开启后本机 + 外发各一次且不重复 / **完全没接 pusher 也必达** / 改期后重新排队 / 单条异常不连累同轮其它 / 外发失败与「通道被清空」兜底都写 `pushStatus` 且点名待办（`wizard.accept` M10、`work.accept` T10、`ui.accept` U12 第 5 步）。
15. **破坏性操作二次确认（v0.16）**：删除工作记录 / 事项 / 知识资料一律**先弹窗后删**，取消不产生任何删除调用（`ui.accept` U13 / U14；harness 支持编译**真子组件**）。
...
| ⑪ | P0 提醒范围 | ✅ v0.4 拍板：只做早上 + 日报两个本地通知，其余 P1；**v0.9 收窄解冻**：时刻可自定义 + 到点自动生成草稿 + 可外发短提示（方案 A），仍不做调度系统 |
