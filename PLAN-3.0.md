# Umi Claw 3.0「工作版」规划基线（v0.15 — 向导入口回归篇）

> 本文档是 3.0 的唯一规划基线，随开发进度持续更新（沿用 `PLAN-2.0.md` 模式）。
> 状态：**规划冻结（v0.9–v0.15 局部解冻：仅提醒外发 + 冷启动弹窗，见修订记录）——产品/系统/工程/数据/IPC/合规全部钉死，开工前终检 8 项已清** ｜ 更新日期：2026-09-24 ｜ 分支：`release/3.0`（基于 `release/1.0`）
...
>
> ⚠️ **护栏（v0.5 立，v0.9 收窄，硬规则 22）**：提醒**仍不是调度系统**——不要加自定义规则、重复规则、多提醒、日历同步。v0.9 放开两项：**时刻可自定义** + **到点自动生成日报草稿**。
>
...
22. **提醒护栏（v0.5 立，v0.9 收窄）**：P0 提醒**仍只做两个本地通知**（早上 + 日报），**仍禁止膨胀成调度系统**（不做自定义规则/重复规则/多提醒/日历同步）。v0.9 放开两项（北 2026-09-23 拍板）：**时刻可自定义**（存 `app_meta`，默认 09:00 / 18:30）+ **到点自动生成日报草稿**。**外发采用方案 A**：只推短提示（= 本地通知同一段 title + body），**绝不推日报正文**（硬规则 5 / 20）；外发**默认关**，未配齐通道 + 目标不得开启。
...
window.api = {
  ...
  work: {
    ...
    reminder: { isEnabled(id), setEnabled(id, enabled),
                ...
                testPush(), getPushStatus(), check() }      // 两个固定通知 + 时刻可改 + 外发（v0.9/v0.10/v0.11 补回写）
  }
}
