...
> 基线版本：v1.34 ｜ 更新日期：2026-09-23 ｜ 状态：**一期 Commit 00-12 全部通过验收；二阶段「分镜脚本生成」落地，真网关冒烟待端点开启后跑一次**

## 修订记录

| 版本 | 日期 | 要点 |
|------|------|------|
| v1.34 | 2026-09-23 | **二阶段：分镜脚本（shooting_script）生成落地**。①DB user_version 1→2——`migration.ts` 只追加 `ALTER TABLE contents ADD COLUMN content_type TEXT NOT NULL DEFAULT 'post'`（CREATE TABLE 保持 v1 形状：新库也走全部 steps，写进建表语句会 duplicate column）；db-worker columns 白名单同步扩列；FTS5+trigram 由 user_version=2 顺延 3（§四检索策略 + 新增「v2 增量迁移」小节 + db-worker 注释同步）。②生成分支：shooting_script = **单路 SSE 流 + 严格 JSON 契约**（temp 0.4，会话键 `content-<id>-script`）；post 维持三路角度。契约 `{title,cover,hook,cta,hashtags,shots[{index,shot,durationSec,voiceover,subtitle,cameraTip}]}`；防护：shots≤30 截断、durationSec 1-300 夹紧（非法→默认 5）、坏 shot 丢弃重编号、hashtags 剥#/去重/≤10。新增纯模块 `shootingScript.ts`（无 electron/HTTP）。③版本/正文分离：版本行存**原始 JSON**（可重渲染），正文由「采用」写回**渲染文本**——GatewayStreamResult/done 增可选 `deliverable`（不影响 advisor/scan），listVersions 对脚本 AI 版本 best-effort 挂 `rendered_content`；解析失败软落地：报错+不落版本行+原文「复制原文」，草稿行保留。④businessLine 从 `project.industry` **推导预选**（含「摄」→摄影、含「服/衣」→服饰、否则空通用），UI 可改；不落 schema 只进 prompt 快照；快照含 JSON 契约+业务线（保住 09 复盘口径）；平台规则走 `renderPackRuleSection` 同源注入（抖音口径：产出脚本文案不是视频）。⑤验收：新增 `test/shooting.accept.mjs` **9/9**，db-worker/db-client 版本断言更新 v2（W2 增 content_type 列断言）；npm 增 `accept:shooting`/`smoke:shooting`；双 typecheck 0 错，14 套 accept 全绿（db 31/31、shooting 9/9、其余 12 套不回归），`npm run build` 通过（ContentCenter 28.6→39.8 kB，零新依赖/零新表）。⑥**提交前复审 8 项修复**（均已验证）：**A1** `forwardGatewayStream` 不再用 `.catch(() => null)` 吞 `handle.result` 的拒绝——落库/解析失败原先被吞成一次「成功 done」，渲染端「采用为正文」的条件（gate `!errorCode`）成立，会把未解析的原始输出写进 `contents.content` 且零版本行可回溯；现透传为 error 事件、已上屏原文保留（「复制原文」兜底），中止引起的拒绝仍由 catch 的 `cancelled` 守卫拦下。**A2** `generate(contentId)` 校验行上 `content_type`：显式类型与草稿冲突 → VALIDATION_ERROR（不传则跟随行上类型）——读取侧 `attachRenderedContent` 按**行上**类型决定是否 `parseShootingScript`，两个真相来源漂移会让版本行永远拿不到 `rendered_content`。**A3** ContentCenter `regenOnEditor` 改用编辑器自带的 `e.content_type`（原从 `filterStatus` 过滤后的列表反查，草稿一旦被过滤掉就退化成 post 三路）。**A4** IPC `ContentGenerateResult` 补回传 `businessLine`（store 一直在读 `res.data?.businessLine`，原先恒 null 的死链路）。**A5** prompt 快照的业务线行只在 shooting_script 时 push（post 从不消费 businessLine，却会在快照里出现「业务线：通用…」污染复盘口径）。**A6** 新增 `jsonExtract.ts` 统一 JSON 提取口径（平衡扫描 + 字符串/转义感知），`shootingScript`/`hotScoreManager`/`advisorManager` 三处共用——原 `extractJsonObject` 的「第一个 `{` 到最后一个 `}`」在解释文字含花括号时必然切错。**A7** `db-client.accept.mjs` 的 user_version 改从 `TARGET_USER_VERSION` 取（原写死 2，FTS5 顺延 v3 时会无故红）。**A8** `package-lock.json` 版本号误改回滚（lock 2.0.0 vs package.json 1.1.0）。验收同步补断言：S4 增**渲染端 error 事件面**（实测还原旧行为即 FAIL）、新增 S10 类型一致性守门、S9 补 IPC **回传侧** `businessLine` 断言（原标题声称覆盖 contentType/businessLine 却只断了 contentType，正是该缺口掩盖了 A4）。另：17 个 `test/*.json` 结果产物 `git rm --cached` 取消跟踪（每次跑测试重写、含本机绝对路径；`/test/` 早已忽略但当时误入库），`.gitignore` 补显式条目。**真网关冒烟**：本次执行到真网关（3213 可达）但 chatCompletions 端点未开启（404 endpoint-disabled），脚本链路已验证；待应用内端点开启后跑一次（不碰真库/不花真商家额度）。⏳ 真 Electron GUI 点击流同一期整体测试；AI 视频提示词 video_prompt 走枚举+模板扩展，下一轮 |
...
### v2 增量迁移（2026-09-23：contents.content_type）

- user_version 1→2；DDL 只在 `migration.ts` 追加：`ALTER TABLE contents ADD COLUMN content_type TEXT NOT NULL DEFAULT 'post'`——CREATE TABLE 保持 v1 形状（新库同样走全部 migration steps，写进建表语句会 duplicate column）。
- 取值：`post`（图文文案，默认；09 的三路角度生成）/ `shooting_script`（分镜脚本；单路 SSE + 严格 JSON 契约，temp 0.4）。
- `content_type` **创建后不可改**（update 白名单不含，同 source_topic_id 先例）；db-worker columns 白名单同步扩列。
- shooting_script 版本行 `content_versions.content` 存**原始 JSON**（可重渲染、可复盘）；正文 `contents.content` 由「采用」写回渲染文本；解析失败不落版本行、已上屏原文保留可复制。
- businessLine（photography/fashion）是生成参数、不落 schema，进 prompt 快照；枚举+模板机制为下轮 video_prompt 留形状。
...
- ~~v2 迁移（user_version=2）：FTS5 + trigram tokenizer~~ **2026-09-23 修订：user_version=2 已被 contents.content_type 占用，FTS5 顺延 user_version=3。**
- v3 迁移（user_version=3）：FTS5 + trigram tokenizer（SQLite ≥ 3.34）。**已确认坑：trigram 对 <3 字符的查询无效，中文双字词（如"减肥"）搜不到——迁移后 LIKE 必须保留为兜底分支**，不能 FTS 化后删 LIKE。FTS5 默认 unicode61 会把整段中文当单 token，不可直接用。
